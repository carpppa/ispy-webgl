var Plotter = (function(){
  ...
  var makeDisc = function(ir, or, pos, slices) {
    var lines = [];
    var points = [];
  ...
  var makeCylinder = function(r, len, pos, slices, segments) {
    var lines = [];
    var points = [];
  ...
  /*
   var makeCylinder = function(rT, rB, height, rSeg, hSeg, openEnded) {
   return new THREE.CylinderGeometry(rT, rB, height, rSeg, hSeg, openEnded);
   }
   */

  var makeTube = function(ir, or, len, pos, slices, segments) {
    var lines = [];
    var points = [];
  ...
  var makeSolidBox = function(data, ci) {
  ...
  var makeScaledSolidBox = function(data, geometry, ci, scale) {
    var f1 = new THREE.Vector3(data[ci][0],   data[ci][1],   data[ci][2]);
    var f2 = new THREE.Vector3(data[ci+1][0], data[ci+1][1], data[ci+1][2]);
    var f3 = new THREE.Vector3(data[ci+2][0], data[ci+2][1], data[ci+2][2]);
    var f4 = new THREE.Vector3(data[ci+3][0], data[ci+3][1], data[ci+3][2]);
  ...
  var makeWireframeBox = function(data, ci) {
    var f1 = new THREE.Vector3(data[ci][0],   data[ci][1],   data[ci][2]);
    var f2 = new THREE.Vector3(data[ci+1][0], data[ci+1][1], data[ci+1][2]);
    var f3 = new THREE.Vector3(data[ci+2][0], data[ci+2][1], data[ci+2][2]);
    var f4 = new THREE.Vector3(data[ci+3][0], data[ci+3][1], data[ci+3][2]);
  ...
  var makeWireFace = function(data, ci) {
    var f1 = new THREE.Vector3(data[ci][0],   data[ci][1],   data[ci][2]);
    var f2 = new THREE.Vector3(data[ci+1][0], data[ci+1][1], data[ci+1][2]);
    var f3 = new THREE.Vector3(data[ci+2][0], data[ci+2][1], data[ci+2][2]);
    var f4 = new THREE.Vector3(data[ci+3][0], data[ci+3][1], data[ci+3][2]);
  ...
  var makeSolidFace = function(data, ci) {
    var f1 = new THREE.Vector3(data[ci][0],   data[ci][1],   data[ci][2]);
    var f2 = new THREE.Vector3(data[ci+1][0], data[ci+1][1], data[ci+1][2]);
    var f3 = new THREE.Vector3(data[ci+2][0], data[ci+2][1], data[ci+2][2]);
    var f4 = new THREE.Vector3(data[ci+3][0], data[ci+3][1], data[ci+3][2]);
  ...
  var makeTrackerPiece = function(data) {
    return makeWireFace(data, 1);
  };

  var makeShapes = function(data) {
    points = data[0];
    lines = data[1];
    shapes = [];
  ...
  var makeModelTrackerBarrel = function(data) {
    ...
    for (var i = 0; i < radii.length; i++) {
      ...
      //var barrel = makeCylinder(r, r, l, slices, 1, true);
  ...
  var makeModelTrackerEndcap = function(data) {
  ...
  var makeModelEcalBarrel = function(data) {
    var hr = data[0];
    var pos = data[1];
    var fr = data[2];
    var hpos = data[3];
  ...
  var makeModelEcalEndcapMinus = function(data) {
    return [makeTube(0.35, 1.5, 0.05, 3.2, 24, 2)];
  };

  var makeModelEcalEndcapPlus = function(data) {
    return [makeTube(0.35, 1.5, -0.05, -3.2, 24, 2)];
  };

  var makeModelEcalPreshower = function(data) {
    return [
      makeDisc(0.4, 1.3, 3.025, 24),
      makeDisc(0.4, 1.3, 3.075, 24),
      makeDisc(0.4, 1.3, -3.025, 24),
      makeDisc(0.4, 1.3, -3.075, 24)
    ];
  };

  var makeModelHcalBarrel = function(data) {
    var points = [];
    var lines = [];
  ...
  var makeModelHcalForward = function(data) {
    return [
      makeTube(0.15, 1.25, 1.7, 11.1, 24, 2),
      makeTube(0.15, 1.25, -1.7, -11.1, 24, 2)
    ];
  };

  var makeModelHcalForwardPlus = function(data) {
    return [makeTube(0.15, 1.25, 1.7, 11.1, 24, 2)];
  };

  var makeModelHcalForwardMinus = function(data) {
    return [makeTube(0.15, 1.25, -1.7, -11.1, 24, 2)];
  };

  var makeModelHcalOuter = function(data) {
    return [
      makeTube(3.9, 4.1, 3, -1.5, 48, 8),
      makeCylinder(4.2, 2.5, 1.5, 24, 7),
  ...
  var makeModelHcalEndcap = function(data) {
    var points = [];
    var lines = [];
  ...
  var makeScaledWireframeBox = function(data, material, ci, scale) {
    var f1 = new THREE.Vector3(data[ci][0],   data[ci][1],   data[ci][2]);
    var f2 = new THREE.Vector3(data[ci+1][0], data[ci+1][1], data[ci+1][2]);
    var f3 = new THREE.Vector3(data[ci+2][0], data[ci+2][1], data[ci+2][2]);
    var f4 = new THREE.Vector3(data[ci+3][0], data[ci+3][1], data[ci+3][2]);
  ...
  var makeTrackPoints = function(data, extra, assoc, style) {
    if ( ! assoc ) {
  ...
  var makeTracks = function(tracks, extras, assocs, style) {
    if ( ! assocs ) {
      throw "No association!";
    }

    var ti, ei;
    var p1, d1, p2, d2, p3, p4;
  ...
  var makeRecHit_V2 = function(data, geometry, scale) {
    var energy = data[0];
    if ( energy > 0.5 ) { // make this a setting
      return makeScaledSolidBox(data, geometry, 5, scale*energy);
    }
  };

  var makeDT = function(dt) {
    //return makeSolidBox(dt, 1);
    return makeWireframeBox(dt, 1);
  };

  var makeCSC = function(csc) {
    //return makeSolidBox(csc, 1);
    return makeWireframeBox(csc, 1);
  };

  var makeMuonChamber = function(chamber) {
    return makeWireframeBox(chamber, 1);
  };

  var makeHcal = function(hb) {
    return makeWireframeBox(hb, 1);
  };

// Should either 1) use BufferGeometry or 2) use border positions to make wireframe
// or 3) something else
  var makeEcal = function(ecal) {
    return makeWireframeBox(ecal, 1);
  };


  var makeRPC = function(rpc) {
    return makeWireFace(rpc, 1);
  };

  var makePointCloud = function(data, index) {
  ...
  var makeTrackingRecHits = function(data) {
    return makePointCloud(data,0);
  };

  var makeTrackingClusters = function(data) {
    return makePointCloud(data,1);
  };

  // This is very inefficient, use point cloud instead
  /*
  var makeTrackingRecHit = function(data,style) {
  ...
  var makeTrackCluster = function(data, style) {
  ...
  var makeMET = function(data, style) {
    /*
  ...
  var makeJet = function(data, style) {
    var et = data[0];
  ...
  var makeDTRecHits = function(data) {
    /*
  ...
  var makeRPCRecHits = function(data) {
  ...
  var makeCSCRecHit2Ds_V2 = function(data, descr) {
    return makeRPCRecHits(data, descr);
  };

  var makeDTRecSegments = function(data) {
    var geometry = new THREE.Geometry();
  ...
  var makeCSCSegments = function(data, geometry) {
    return makeDTRecSegments(data, geometry);
  };

  var makeCSCDigis = function(data, w, d, rotate) {
  ...
  var makeCSCWireDigis = function(data) {
    return makeCSCDigis(data, 0.02, 0.01, Math.PI*0.5);
  };

  var makeCSCStripDigis = function(data) {
  ...
  var makeEvent = function(data) {
    ...
    $('#cms-logo').html("<img src='./graphics/cms-color-medium.png'></img>");
  ...
  return {
    ...
    makeCSC: makeCSC,
    makeDT: makeDT,
    makeModelHcalForwardMinus: makeModelHcalForwardMinus,
    makeModelHcalForwardPlus: makeModelHcalForwardPlus,
    makeHcal: makeHcal,
    makeModelHcalOuter: makeModelHcalOuter,
    makeModelHcalEndcap: makeModelHcalEndcap,
    makeModelHcalBarrel: makeModelHcalBarrel,
    makeModelEcalEndcapMinus: makeModelEcalEndcapMinus,
    makeModelEcalEndcapPlus: makeModelEcalEndcapPlus,
    makeModelEcalBarrel: makeModelEcalBarrel,
    makeEcal: makeEcal,
    makeModelTrackerEndcap: makeModelTrackerEndcap,
    makeModelTrackerBarrel: makeModelTrackerBarrel,
    makeTrackerPiece: makeTrackerPiece,
    makeRecHit_V2: makeRecHit_V2,
    makeTracks: makeTracks,
    makeTrackingRecHits: makeTrackingRecHits,
    makeTrackingClusters: makeTrackingClusters,
    makeEvent: makeEvent,
    makeDTRecHits: makeDTRecHits,
    makeDTRecSegments: makeDTRecSegments,
    makeRPCRecHits: makeRPCRecHits,
    makeCSCStripDigis: makeCSCStripDigis,
    makeCSCWireDigis: makeCSCWireDigis,
    makeCSCRecHit2Ds_V2: makeCSCRecHit2Ds_V2,
    makeCSCSegments: makeCSCSegments,
    makeMuonChamber: makeMuonChamber,
    makeMET: makeMET,
    makeJet: makeJet,
    makePhoton: makePhoton,
    makeTrackPoints: makeTrackPoints,
